...
DIR_TARGET=/home/pi/SmartPowerManager
RPI_USER_NAME="pi@"
SLASH_VAR="/"
...
echo -e "${YELLOW}Enter Raspberry Pi IP adress (Main board) : ${ENDCOLOR}"
read RPI_TARGET
echo ""
...
case "$response" in
    [yY][eE][sS]|[yY]) 
		...
		#------copy file------
		rsync -r ./SmartPowerManager/WebServeur $RPI_USER_NAME$RPI_TARGET:$DIR_TARGET --progress &&
...
ssh $RPI_USER_NAME$RPI_TARGET 'bash -s' <<ENDSSH
  ...
  crontab -l | { cat; echo "@reboot cd /home/pi/SmartPowerManager/WebServeur/ ; sudo bash start.sh"; } | crontab -
  crontab -l | { cat; echo "@reboot cd /home/pi/SmartPowerManager/WebServeur/ ; sudo bash start.sh"; } | crontab -
  sudo reboot
